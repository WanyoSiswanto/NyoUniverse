...
use App\Filament\Resources\ProgramQualificationResource\Pages;
...
class ProgramQualificationResource extends Resource
{
    protected static ?string $model = ProgramQualification::class;
    ...
    {
        return __('messages.qualification_programs');
    }

    public static function getModelLabel(): string
    {
        return __('messages.qualification_program');
    }
        ...
        return $form
            ->schema([
                Forms\Components\Select::make('master_id')
                    ...
                    ->options(MasterQualification::where('is_active', true)->pluck('name', 'id'))
                    ->searchable()
                    ->required()
                ...
                Forms\Components\Select::make('status')
                    ->options(ProgramQualification::STATUS_OPTIONS)
                    ->required()
                    ->default('pending')
        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('approve')
                    ...
                    ->visible(fn (ProgramQualification $record): bool => $record->status === 'pending' && Auth::user()->hasRole(['admin', 'manager']))
                    ->action(function (ProgramQualification $record): void {
                        $record->update([
                            'status' => 'approved',
                            'approved_by' => Auth::id(),
                            'approved_at' => now(),
    ...
    {
        return [
            'index' => Pages\ListProgramQualifications::route("/"),
            'create' => Pages\CreateProgramQualification::route("/create"),
            'edit' => Pages\EditProgramQualification::route("/{record}/edit"),
        ];
    }
}
